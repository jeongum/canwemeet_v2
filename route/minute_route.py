...
def minute_details():
    ...
    stt = minutedb.db.engine.execute("SELECT hour(rstt.time) AS hour, minute(rstt.time) AS minute, rstt.content, em.emotion_name, user.user_name FROM realtime_STT as rstt INNER JOIN user ON rstt.user_id=user.user_id inner join realtime_emotion as re on rstt.stt_id = re.stt_id inner join emotion as em on re.emotion_id = em.emotion_id where rstt.room_id = %s ORDER BY rstt.stt_id", (board_id))
