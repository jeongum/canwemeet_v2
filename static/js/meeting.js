        ...
        recognition.onresult = function(e) {
            for(let i = e.resultIndex, len = e.results.length; i < len; i++)
                if(e.results[i].isFinal)
                    index = i;
                    transcript = e.results[index][0].transcript;
                    $('input#wav_index').val(index);
                    get_emotion();
...
function get_emotion(){
    ...
    $.ajax({
        ...
        success : function(result){
            ...
            $('#emotion-image li.current_icon').children('img').attr("src", "/static//images/meeting/icon-"+result+"-mono.png");
            $('#emotion_image li.current_icon').removeClass('current_icon')

            $('#graph-arrow li#'+result+'-arrow').addClass('current_arrow');
            $('#graph-arrow li.current_arrow').html(arrow_img);
            $('#emotion-image li#'+result+'-icon').children('img').attr("src", "/static//images/meeting/icon-"+result+".png");
            $('#emotion-image li#'+result+'-icon').addClass('current_icon');
        },
